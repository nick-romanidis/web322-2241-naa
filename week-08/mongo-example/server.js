...
app.get("/", (req, res) => {
    nameModel.find()
        ...
        .catch(err => {
            console.log("Couldn't get list of names. " + err);
...
app.post("/addName", (req, res) => {
    ...
    nameNew.save()
        .then(() => {
            console.log("Saved the name " + nickname);
            res.redirect("/");
        })
        .catch(err => {
            console.log("Couldn't save the name " + nickname + "\n" + err);
            res.redirect("/");
        });
});
...
app.post("/updateName", (req, res) => {
    ...
    if (fName.trim().length === 0 && lName.trim().length === 0) {
        ...
        nameModel.deleteOne({
            ...
            .then(() => {
                console.log("Deleted the name " + nickname);
                res.redirect("/");
            })
            .catch(err => {
                console.log("Couldn't delete the name " + nickname + "\n" + err);
                res.redirect("/");
            });
    }
    else {
